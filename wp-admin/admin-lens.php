...
$sellens = "select *, left(keterangan, 100) as keterangan_produk from lensa order by last_update desc";
...
<br>
	<div class="w3-right"><a href="http://www.c-fielien.com/lenses" target="blank"><i class="fa fa-external-link"></i> Lihat Website</a></div>
<br><br>	
...
<center>
	...
	<div class="w3-border-tab w3-border-yellow" style="width:30%;height: 5px" ></div>
		...
		while ($rowlens = $reslens->fetch_assoc()) {
			...
			echo "<td>".$counter."</td>";
			echo '<td>'.$rowlens['nama_lensa'].'</td>';
			echo '<td>'.$rowlens['keterangan_produk'].'</td>';
			echo '<td>'.$rowlens['picture_lensa'].'</td>';
			echo '</tr>';
			$counter++;
		}
		?>
	...
	<div class="w3-border-tab w3-border-pale-blue" style="width:30%;height: 5px" ></div></center>
		...
		while ($rowedlens = $resedlens->fetch_assoc()) {
			$idlens=$rowedlens['id_lensa'];
			# code...
			echo '<tr>';
			//echo '<td>'.$rowedlens['id_lensa'].'</td>';
			echo "<td>".$counter."</td>";
			echo '<td>'.$rowedlens['nama_lensa'].'</td>';
			echo '<td>'.$rowedlens['keterangan_produk'].'</td>';
			echo '<td>'.$rowedlens['picture_lensa'].'</td>';
			
			echo '<td><button class="w3-btn w3-purple" value="'.$idlens.'" onclick="editLensCategory(this.value)">Edit</button></td>';
			echo '</tr>';
			$counter++;
		}
		?>
	...
	<div class="w3-border-tab w3-border-yellow w3-right" style="width:30%;height: 5px" ></div>
		...
		while ($rowdellens = $resdellens->fetch_assoc()) {
			$iddellens=$rowdellens['id_lensa'];
			# code...
			echo '<tr>';
			//echo '<td>'.$rowdellens['id_lensa'].'</td>';
			echo "<td>".$counter."</td>";
			echo '<td>'.$rowdellens['nama_lensa'].'</td>';
			echo '<td>'.$rowdellens['keterangan_produk'].'</td>';
			echo '<td>'.$rowdellens['picture_lensa'].'</td>';
			
			echo '<td>
						<form method = "post" action="master-lens.php">
